...
namespace Web.Services
{
    public class HomeIndexViewModelService : IHomeIndexViewModelService
    {
        ...
        public async Task<List<SelectListItem>> GetBrands()
        {
            ...
            var items = brands.Select(x => new SelectListItem() { Value = x.Id.ToString(), Text = x.BrandName })
                .OrderBy(x => x.Text)
                .ToList();
        ...
        public async Task<List<SelectListItem>> GetCategories()
        {
            ...
            var items = categories.Select(x => new SelectListItem() { Value = x.Id.ToString(), Text = x.CategoryName })
                .OrderBy(x => x.Text)
                .ToList();
